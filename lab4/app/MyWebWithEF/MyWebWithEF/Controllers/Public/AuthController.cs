...
namespace MyWebWithEF.Controllers.Public
{
    public class AuthController : PublicApiController
    {
        ...
        public IActionResult Login([FromBody] LoginRequest request)
        {
            ...
            if (username == "user" && request.Password == "pass")
            {
                var token = GenerateJwtToken(username, UserRole.User);
                return Ok(new { Token = token });
            }


            if (username == "admin" && request.Password == "pass")
            {
                var token = GenerateJwtToken(username, UserRole.Admin);
                return Ok(new { Token = token });
            }
            return Unauthorized();
        }
